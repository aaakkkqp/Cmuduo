...
{
    if (buf.GetAvail() >= kMaxNumericSize)
    {
        int len = snprintf(buf.GetCur(), kMaxNumericSize, "%.12g", v); 
        buf.add(len);
        return *this;
    }
}
...
LogStream& LogStream::operator<<(const Buffer& buff)
{
    *this << buff.toString();
    return *this;
}
